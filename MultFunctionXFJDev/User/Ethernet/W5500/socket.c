...
uint8 socket(SOCKET s, uint8 protocol, uint16 port, uint8 flag)
{
      ...
      )
   {
      ...
      if (port != 0) {
         ...
         Config.local_port ++; // if don't set the source port, set local_port number.
         IINCHIP_WRITE(Sn_PORT0(s) ,(uint8)((Config.local_port & 0xff00) >> 8));
         IINCHIP_WRITE(Sn_PORT1(s) ,(uint8)(Config.local_port & 0x00ff));
      }
